...
export function SiteHeader() {
  ...
  useEffect(() => {
    // Fetch dynamic GitHub stars for reloop-templates (or fallback to reloop repo)
    fetch("https://api.github.com/repos/reloop-labs/reloop-templates")
      .then((res) => res.json())
      .then((data) => {
      ...
      <div className="flex h-14 w-full items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex h-8 items-center gap-1.5 rounded-md border border-zinc-200 bg-white px-2.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-300 dark:hover:bg-zinc-900 transition-colors"
            title="Star reloop-templates on GitHub"
          >
            ...
            <span className="font-mono">
              {starCount !== null ? `${starCount} ★` : "GitHub"}
            </span>
          </a>
